...
function buildValidationMessage(errors) {
  if (!Array.isArray(errors) || errors.length === 0) {
    return "The resource could not be created because some fields are invalid. Please check the form and try again.";
  }
  ...
  return `The resource could not be created because some fields are invalid. Please check the highlighted fields and try again.\n\n${lines.join("\n")}`;
}
...
async function onSubmit(event) {
  ...
  try {
    ...
    const resourceName = body?.data?.name ?? payload.resourceName ?? "Resource";
    const msg = `Resource "${resourceName}" was added successfully.`;
