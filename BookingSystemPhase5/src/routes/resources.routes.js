...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    await logEvent({
    actorUserId,
    message: `Resource created successfully: name="${resourceName}", price=${Number(resourcePrice).toFixed(2)}, unit="${resourcePriceUnit}"`,
    entityType: "resource",
    entityId: resourceId,
});
    ...
    if (err && err.code === "23505") {
     ...
     await logEvent({
     actorUserId,
     message: `Resource creation blocked: duplicate name "${resourceName}" (409)`,
     entityType: "resource",
     entityId: null,
});
